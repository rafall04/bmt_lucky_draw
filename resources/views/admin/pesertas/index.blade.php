<x-admin-layout title="Data Peserta">
    <!-- Header -->
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        <form method="GET" action="{{ route('admin.pesertas.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Status</label>
                <select name="status" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    <option value="all" {{ request('status') === 'all' || !request('status') ? 'selected' : '' }}>Semua Peserta</option>
                    <option value="menang" {{ request('status') === 'menang' ? 'selected' : '' }}>Sudah Menang</option>
                    <option value="belum_menang" {{ request('status') === 'belum_menang' ? 'selected' : '' }}>Belum Menang</option>
                </select>
            </div>
    ...
    <!-- Bulk Actions -->
    <form id="bulkDeleteForm" action="{{ route('admin.pesertas.bulk-delete') }}" method="POST" class="mb-4">
        @csrf
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-green-600 to-green-700">
            <h2 class="text-xl font-bold text-white">Total: {{ $pesertas->total() }} Peserta</h2>
        </div>
                        ...
                        <tr class="hover:bg-green-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('admin.pesertas.edit', $peserta) }}" 
                                       class="text-blue-600 hover:text-blue-900" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.pesertas.destroy', $peserta) }}" 
                                          method="POST" 
                                          class="inline"
