<x-admin-layout title="Tambah Peserta">
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-2">Tambahkan peserta undian secara manual</p>
        </div>

        <div class="bg-white rounded-xl shadow-lg p-8">
            <form action="{{ route('admin.pesertas.store') }}" method="POST">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="alamat" 
                                  rows="3"
                                  required
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('alamat') }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
